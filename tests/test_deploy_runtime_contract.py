...
def _dev2_firebase_env(*extra: str) -> str:
    return chr(10).join(
        [
            "ARCHBRO_ENV=production",
            "ARCHBRO_AUTH_MODE=firebase",
...
def test_dev2_provider_oauth_requires_matching_secret(tmp_path: Path) -> None:
    result = _validate(
        tmp_path,
        "archbro-dev2",
        _dev2_firebase_env("ARCHBRO_GITHUB_OAUTH_CLIENT_ID=github-client"),
    )
    assert result.returncode != 0
...
def test_dev2_provider_oauth_requires_stable_credential_key(tmp_path: Path) -> None:
    ...
    assert result.returncode != 0
    assert "ARCHBRO_PROVIDER_CREDENTIAL_KEY" in result.stdout + result.stderr


def test_dev2_provider_oauth_accepts_durable_encrypted_contract(tmp_path: Path) -> None:
    result = _validate(
        tmp_path,
        "archbro-dev2",
        _dev2_firebase_env(
            "ARCHBRO_GITHUB_OAUTH_CLIENT_ID=github-client",
            "ARCHBRO_GITHUB_OAUTH_CLIENT_SECRET=github-secret",
            "ARCHBRO_PROVIDER_CREDENTIAL_KEY=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=",
...
def test_dev2_rejects_session_only_provider_oauth_escape_hatch(tmp_path: Path) -> None:
    result = _validate(
        tmp_path,
        "archbro-dev2",
        _dev2_firebase_env("ARCHBRO_ALLOW_EPHEMERAL_PROVIDER_OAUTH=true"),
    )
    assert result.returncode != 0
    assert "must not enable ARCHBRO_ALLOW_EPHEMERAL_PROVIDER_OAUTH" in result.stdout + result.stderr
